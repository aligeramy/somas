        ...
        <PopoverContent className="w-64 p-3 rounded-xl" align="start">
          <div className="space-y-3">
            <div className="text-sm">
              ...
              {selectedDate && getOccurrenceForDate(selectedDate) ? (
                <Badge
                  variant={getOccurrenceForDate(selectedDate)?.status === "scheduled" ? "default" : "destructive"}
                  className="mt-1"
                >
                  {getOccurrenceForDate(selectedDate)?.status === "scheduled" ? "Scheduled" : "Canceled"}
                  {getOccurrenceForDate(selectedDate)?.isCustom && " (Custom)"}
                </Badge>
              ) : (
            ...
            <div className="flex flex-col gap-2">
              {selectedDate && getOccurrenceForDate(selectedDate) ? (
                <>
                  {getOccurrenceForDate(selectedDate)?.status === "scheduled" && onToggleDate && (
                    <Button
                      size="sm"
                      variant="outline"
                  ...
                  {getOccurrenceForDate(selectedDate)?.status === "canceled" && onToggleDate && (
                    <Button
                      size="sm"
                      variant="outline"
                  ...
                  {getOccurrenceForDate(selectedDate)?.isCustom && onRemoveDate && (
                    <Button
                      size="sm"
                      variant="destructive"
                      className="w-full justify-start gap-2 rounded-lg"
