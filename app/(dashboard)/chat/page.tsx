...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    async function loadChannels() {
      try {
        ...
        if (response.ok) {
          // Include all channels (global, dm, and group chats including event group chats)
          const allChannels = result.channels || [];
          // Sort channels: global first, then others
          const sortedChannels = allChannels.sort((a: Channel, b: Channel) => {
            if (a.type === "global") {
              return -1;
            }
            if (b.type === "global") {
  ...
  const createOrOpenDM = useCallback(
    async (userId: string) => {
      // Prevent self-DM (client-side validation)
  ...
  async function handleCreateChannel() {
    ...
    // Prevent self-DM (client-side validation)
    if (chatType === "dm" && currentUser && targetUserId === currentUser.id) {
      alert("Cannot create a direct message with yourself");
      return;
    }
                            ...
                            >
                              <div className="flex min-w-0 flex-1 items-center gap-2">
                                ...
                                <div className="flex min-w-0 flex-1 items-center gap-1.5">
                                  ...
                                  <Badge
                                    className="shrink-0 rounded-sm border-blue-500/30 bg-blue-500/20 text-blue-600 text-xs dark:text-blue-400"
                                    variant="secondary"
                                  >
                                    Public
                                  </Badge>
