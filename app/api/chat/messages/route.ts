...
import { users, messages, channels, chatNotifications } from "@/drizzle/schema";
...
export async function POST(request: Request) {
  try {
    ...
    if (!dbUser || !dbUser.gymId) {
      console.log("[POST /api/chat/messages] User has no gym:", { dbUser: !!dbUser, gymId: dbUser?.gymId });
      return NextResponse.json(
    ...
    console.log("[POST /api/chat/messages] Request body:", { channelId, contentLength: content?.length, hasAttachment: !!attachmentUrl });

    if (!channelId || !content) {
      console.log("[POST /api/chat/messages] Missing required fields:", { channelId: !!channelId, content: !!content });
      return NextResponse.json(
    ...
    console.log("[POST /api/chat/messages] Message inserted:", { id: newMessage.id, channelId: newMessage.channelId });
    ...
    if (!messageWithSender) {
      console.error("[POST /api/chat/messages] Failed to fetch created message");
      return NextResponse.json(
    ...
    console.log("[POST /api/chat/messages] Message with sender fetched successfully");
    ...
    console.log("[POST /api/chat/messages] Fetching target users for notifications");
    ...
    console.log("[POST /api/chat/messages] Found target users (before filtering):", targetUsers.length);

    // For DM channels, filter to only the other participant
    // (DM channel name is the other user's name or email)
    if (channel.type === "dm") {
      console.log("[POST /api/chat/messages] Filtering for DM channel, channel name:", channel.name);
      const beforeFilter = targetUsers.length;
      targetUsers = targetUsers.filter(
        (u) => (u.name && u.name === channel.name) || u.email === channel.name
      );
      console.log("[POST /api/chat/messages] Filtered DM users:", { before: beforeFilter, after: targetUsers.length });
    }

    // Create notification records for all target users
    console.log("[POST /api/chat/messages] Creating notification records for", targetUsers.length, "users");
    ...
    if (notificationRecords.length > 0) {
      try {
        ...
        console.log("[POST /api/chat/messages] Notification records inserted successfully");
      } catch (notifError) {
        console.error("[POST /api/chat/messages] Error inserting notification records:", notifError);
        // Don't fail the whole request if notifications fail
      }
    } else {
      console.log("[POST /api/chat/messages] No notification records to insert");
    }
    ...
    console.log("[POST /api/chat/messages] Filtering users for push notifications");
    ...
    console.log("[POST /api/chat/messages] Push tokens found:", pushTokens.length);
    ...
    if (pushTokens.length > 0 && channel.id && newMessage.id) {
      ...
      console.log("[POST /api/chat/messages] Sending push notifications:", {
        ...
        title: channel.type === "dm" ? senderName : `${senderName} in ${channelName}`,
      ...
      sendPushNotification(
        ...
        console.error("[POST /api/chat/messages] Error sending push notifications:", error);
      });
    } else {
    ...
    console.log("[POST /api/chat/messages] Message created successfully, returning response");
    return NextResponse.json({ message: messageWithSender });
  } catch (error) {
    console.error("[POST /api/chat/messages] Message creation error:", error);
    console.error("[POST /api/chat/messages] Error stack:", error instanceof Error ? error.stack : "No stack trace");
